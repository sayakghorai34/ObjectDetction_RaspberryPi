...
app.post("/upload/image", upload.single("image"), async (req, res) => {
  try {
    ...
    console.error(error);
    res.render("index", { message: "Failed to process image.", result: null, imgData: null, classCounts: null });
  } finally {
...
app.post("/upload/video", upload.single("video"), async (req, res) => {
  try {
    ...
    res.render("index", {
      ...
      videoData: videoSrc,
      imageData: null,
      classCounts: null, // No class counts for video
    });
  } catch (error) {
    console.error(error);
    res.render("index", { message: "Failed to process video.", videoData: null, classCounts: null });
  } finally {
    fs.unlinkSync(req.file.path); // Clean up the uploaded file
  }
});
